...
def expand_single_feature_to_export_model(x, expanded_features):
    for key in expanded_features:
        x[key] = tf.expand_dims(x[key],0)
    return x

def parse_sequence_example(example, features_config, truncate_sequence_length=20,predict_only=False):
...
def parse_sequence_example_predict(example, features_config, truncate_sequence_length=20):
    ...
    expanded_features = list(features_config['sequence_features'].keys())
    a = expand_single_feature_to_export_model(merged_expanded_features,expanded_features)
    print(a)
    expanded_features = list(features_config['single_features'].keys())
    b = deflate_and_split_features_label(a, expanded_features)
...
def make_dataset(path, features_config, batch_size=128, num_map_threads=None,
                 truncate_sequence_length=20, predict_only=False):
    ...
    dataset =  dataset.map(lambda x: parse_sequence_example(x, features_config,
                                                            truncate_sequence_length=truncate_sequence_length,predict_only=predict_only),
...
def make_dataset_generator(gen, features_config, batch_size=128, num_map_threads=None,
                 truncate_sequence_length=20, predict_only=False):
    ...
    # def gen():
    #     yield path
        # yield tf.data.TFRecordDataset(path, compression_type='GZIP')
...
def prepare_dataset_iterator(files, features_config, batch_size=128,
                            truncate_session_length=20,predict_only=False):
    with tf.device('/cpu:0'):
        # Make a dataset 
        ds = make_dataset(files, features_config, batch_size=batch_size,
                            truncate_sequence_length=truncate_session_length,predict_only=predict_only)
...
def prepare_dataset_generator_predict(generator, features_config, batch_size=128,
                            truncate_session_length=20,predict_only=False):
    with tf.device('/cpu:0'):
        # Make a dataset 
        ds = make_dataset_generator(generator, features_config, batch_size=batch_size,
                            truncate_sequence_length=truncate_session_length,predict_only=predict_only)
