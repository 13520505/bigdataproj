...
def write_eval_data(data_eval, folder_name):
    base = os.path.dirname(folder_name)
    if not os.path.exists(base):
        os.makedirs(base)
    if not folder_name:
        folder_name = "./tmp"+datetime.datetime.now().strftime('%Y%m%d%H%M%S')
    np.savetxt(folder_name+".csv", data_eval, delimiter=",", fmt="%s")
