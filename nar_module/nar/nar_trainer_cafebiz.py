...
from acr_module.acr.acr_module_service import get_all_file, get_file_max_date, load_json_config
from nar_module.nar.preprocessing.nar_preprocess_cafebiz_2 import delete_all_file_in_path
from .utils import deserialize, resolve_files, chunks, merge_two_dicts, log_elapsed_time, append_lines_to_text_file, min_max_scale, get_current_time
...
def build_estimator(run_config, model_dir,
...
def split_string_train_path(string):
    list_path = string.split('/')[:-1]
    stringa = '/'.join(list_path) + "/"
    return stringa


def get_train_file_max(path):
    return len(get_all_file(path))
...
def main(unused_argv):
    try:
        ...
        paht_session_spark = data_dir + list_args["input_sessions_json_folder_path"] +"/"
        ...
        nar_label_encoders=load_nar_module_preprocessing_resources(get_all_file(data_dir + FLAGS.nar_module_preprocessing_resources_path)[0])
        ...
        train_files_from = 0
        train_files_up_to = get_train_file_max(split_string_train_path(data_dir + FLAGS.train_set_path_regex))
        ...
        delete_all_file_in_path(split_string_train_path(data_dir + FLAGS.train_set_path_regex))
        import shutil
