...
from .benchmarks import RecentlyPopularRecommender, ContentBasedRecommender, ItemCooccurrenceRecommender, \
    ItemKNNRecommender, SessionBasedKNNRecommender, SequentialRulesRecommender, Word2VecKNN

from .nar_utils import load_acr_module_resources, load_nar_module_preprocessing_resources, \
...
tf.flags.DEFINE_float('recent_clicks_buffer_hours', default=1.0,
                      help='Number of hours that will be kept in the recent clicks buffer (limited by recent_clicks_buffer_max_size)')
tf.flags.DEFINE_integer('recent_clicks_buffer_max_size', default=500, help='Maximum size of recent clicks buffer')
tf.flags.DEFINE_integer('recent_clicks_for_normalization', default=500,
                        help='Number of recent clicks to normalize recency and populary  novelty) dynamic features')
...
tf.flags.DEFINE_float('diversity_reg_factor', default=0.0,
                      help='Diversity (similarity) Regularization Loss (e.g. 0.1, 0.2, 0.3)')
tf.flags.DEFINE_float('eval_negative_sample_relevance', default=0.1,
                      help='Relevance of negative samples within top-n recommended items for evaluation (relevance of positive sample is always 1.0)')

tf.flags.DEFINE_list('enabled_clicks_input_features_groups',
                     default='ALL',
                     help='Groups of input contextual features for user clicks, separated by comma. Valid values: ALL,NONE,time,device,location,referrer')
tf.flags.DEFINE_list('enabled_articles_input_features_groups',
                     default='ALL',
                     help='Groups of input metadata features for articles, separated by comma. Valid values: ALL,NONE,category,author')
tf.flags.DEFINE_list('enabled_internal_features',
                     default='ALL',
                     help='Internal features. Valid values: ALL,NONE,recency,novelty,article_content_embeddings,item_clicked_embeddings')
...
tf.flags.DEFINE_integer('save_results_each_n_evals', default=5,
...
tf.flags.DEFINE_bool('save_eval_sessions_recommendations', default=False,
                     help='Save CHAMELEON recommendations log during evaluation')
...
def nar_module_model_fn(features, labels, mode, params):
    ...
    if not FLAGS.disable_eval_benchmarks:
        eval_benchmark_classifiers = [
            {'recommender': Word2VecKNN, 'params': {"total_epoch": 100, "window": 5}},
            ...
            #             'candidate_sessions_sample_size': 2000,
            #             # 200, #Number of candidate near sessions to sample
            #             'sampling_strategy': 'recent',  # (recent,random)
            #             'nearest_neighbor_session_for_scoring': 500,
            #             # 50 #Nearest neighbors to compute item scores
...
def save_sessions_negative_items(model_output_dir, sessions_negative_items_list,
                                 output_file='eval_sessions_negative_samples.json'):
...
def main(unused_argv):
    try:
        ...
        for chunk_id in tqdm(range(0, len(training_files_chunks) - 1)):
        ...
        if FLAGS.save_eval_sessions_recommendations:
            # Flushing to disk the recommended items to test re-ranking approaches (e.g. MMR)
            save_sessions_chameleon_recommendations_log(model_output_dir, sessions_chameleon_recommendations_log,
                                                        global_eval_hour_id)
