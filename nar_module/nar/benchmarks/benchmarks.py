...
from ..utils import write_eval_data
...
class BenchmarkRecommender:
    ...
    def evaluate(self, users_ids, sessions_items, sessions_next_items, save_path, topk=5, eval_negative_items=None):
        ...
        #Write preds to files
        #MINH
        if save_path:
            save_path += self.get_clf_suffix()
            predicted_item_convert = np.array(
                ["|".join(",".join(map(str, pred_item)) for pred_item in predict) for predict in list(preds)])

            write_eval_data(predicted_item_convert, save_path)
    ...
    def _get_top_n_valid_items(self, items, topk, valid_items):
        ...
        for i in range(count, topk):
            yield 0 
    def _get_top_n_valid_items_no_pad(self, items, topk, valid_items):
        count = 0
        if valid_items is None:
            for item in items:
                if count == topk:
            ...
            for item in items:
                ...
                if (item in valid_items): #or (valid_items is None):
                    count += 1
                    yield item
    def _get_top_n_filter_items_no_pad(self, items, topk, valid_items, filter_items):
        count = 0
        if valid_items is None:
            for item in items:
                if count == topk:
            ...
            for item in items:
                if count == topk:
                    break
                if (item in valid_items) and (item not in filter_items): #or (valid_items is None):
                    count += 1
                    yield item
