...
from .evaluation import (ColdStartAnalysisState, compute_metrics_results,
                         update_metrics)
from .metrics import (
    MRR, NDCG, ContentExpectedRankRelativeRelevanceSensitiveIntraListDiversity,
    ContentExpectedRankRelativeSensitiveIntraListDiversity,
    ExpectedRankRelevanceSensitiveNovelty, ExpectedRankSensitiveNovelty,
    HitRate, HitRateBySessionPosition, ItemCoverage)
from .pageview_pb2 import LastNClickRequest
from .pageview_pb2_grpc import ArticleContextServiceStub
from .utils import get_tf_dtype, merge_two_dicts, write_eval_data
...
#Get list_id from google service
def get_list_id():
    # with grpc.insecure_channel('10.5.36.95:8059') as channel:
    #     stub = ArticleContextServiceStub(channel)
    #     request = LastNClickRequest(n=5000)
    #     a = stub.GetLastNClick(request)
    #     list_id = a.newsId
        # set_id = set(list_id)
        # print("SET:===========================")
        # print(set_id)
    ...
    #
    import random
    list_id = [random.randint(0,1000) for i in range(5000)]
    return list_id
        ...
        with tf.variable_scope("main", initializer=xavier_initializer()):
            ...
            with tf.variable_scope("inputs"):
                ...
                if mode == tf.estimator.ModeKeys.PREDICT:
                    self.label_last_item = None
                    all_clicked_items = item_clicked
                    self.next_item_label = None
                else:
            ...
            with tf.variable_scope("recommendations_ranking"): 
                ...
                if mode == tf.estimator.ModeKeys.PREDICT:
                    scope_name = "cos_sim_positive"
                else:
                    scope_name = "cos_sim_negative"
                ...
                with tf.variable_scope("softmax_function"):                    
                    ...
                    if mode == tf.estimator.ModeKeys.PREDICT:
                        cos_sim_concat = cos_sim_negative
                    else:
                ...
                if mode == tf.estimator.ModeKeys.PREDICT:
                    pos_neg_items_ids = batch_negative_items
                    predicted_item_ids, predicted_item_probs = self.rank_items_by_predicted_prob(pos_neg_items_ids, items_prob)
                    self.predicted_item_ids = predicted_item_ids
                    self.predicted_item_probs = predicted_item_probs
                    self.predictions = {
                        "top_k_predictions":predicted_item_ids[:,:,:self.metrics_top_n],
                        "top_k_prob":predicted_item_probs[:,:,:self.metrics_top_n],
...
class ClickedItemsState:
    ...
    def get_recent_pop_norm_for_predict(self, list_id): #tungtv
        # test
        # list_id  = list(np.random.randint(low=1, high=2800, size=100))
        # print("list_id")
        # print(list_id)
        recent_clicks_item_counter = Counter(list_id)

        articles_recent_pop = np.zeros(shape=[self.num_items], dtype=np.int64)
        # print(self.num_items)
        # print(len(list(recent_clicks_item_counter.keys())))
        # print(len(list(recent_clicks_item_counter.values())))

        # # tungtv
        # lent = len(list(recent_clicks_item_counter.keys()))
        # for i in range(0, self.num_items):
        #     if i < lent:
        #         articles_recent_pop[i] = list(recent_clicks_item_counter.values())[i]

        articles_recent_pop[list(recent_clicks_item_counter.keys())] = list(recent_clicks_item_counter.values())
        # articles_recent_pop = np.zeros(shape=[len(recent_clicks_item_counter.keys())], dtype=np.int64)
        # articles_recent_pop = list(recent_clicks_item_counter.values())

        # articles_recent_pop = np.asarray(articles_recent_pop)

        min_norm_pop = 1.0 / self.recent_clicks_for_normalization
        articles_recent_pop_norm = np.maximum(articles_recent_pop / (articles_recent_pop.sum() + 1),
                                              [min_norm_pop])

        b = np.asarray(list_id)
        b = b.reshape(len(b), 1)
        a = np.zeros(shape=[self.recent_clicks_buffer_max_size - len(b), 1], dtype=np.int64)
        recent_click_bufer = np.vstack([b,a])
        recent_click_bufer = recent_click_bufer[0:self.recent_clicks_buffer_max_size].reshape(self.recent_clicks_buffer_max_size,)
        # print(articles_recent_pop_norm)
        # print(len(articles_recent_pop_norm))
        #
        # print(recent_click_bufer)
        # print(len(set(recent_click_bufer)))

        lista = []
        lista.append(articles_recent_pop_norm)
        lista.append(recent_click_bufer)
...
class ItemsStateUpdaterHook(tf.train.SessionRunHook):
    ...
    def __init__(self, mode, model, eval_metrics_top_n, 
        ...
        self.global_eval_hour_id = global_eval_hour_id
    ...
    def before_run(self, run_context): 
        ...
        if self.mode == tf.estimator.ModeKeys.EVAL:
            ...
            fetches['predicted_item_probs'] = self.model.predicted_item_probs
        # create feed_dict for each ModeKeys
        if self.mode == tf.estimator.ModeKeys.TRAIN or self.mode == tf.estimator.ModeKeys.EVAL:
            ...
            start = time()
            # pass
            # if mode = ModeKeys.PREDICT, feed_dict from service
            # list_id = get_list_id() # edit read from google service

            from pick_singleton.pick_singleton import ACR_Pickle_Singleton
            acr_pickle = ACR_Pickle_Singleton().getInstance()
            list_id =acr_pickle.list_id
            #convert news id to encoded id
            encoded_list_id = []
            count = 0
            # print("list_id_before")
            # print(list_id)

            # print(len(acr_pickle.acr_label_encoders['article_id'].keys()))

            for id in list_id:
                if(int(id) in acr_pickle.acr_label_encoders['article_id']):
                    encoded_list_id.append(acr_pickle.get_article_id_encoded(int(id)))
                else:
                    count = count +1


            # print("+++++++++++++++++++++++++")
            recency = self.clicked_items_state.get_recent_pop_norm_for_predict(encoded_list_id)
            # print("BEFORE RUN TIME: "+str(time()-start))
            feed_dict = {
                self.model.articles_recent_pop_norm: recency[0],
                self.model.pop_recent_items_buffer: recency[1],
                # self.model.articles_recent_pop_norm: self.clicked_items_state.get_articles_recent_pop_norm(),
                # self.model.pop_recent_items_buffer: self.clicked_items_state.get_recent_clicks_buffer(),
                # Passed as placeholder (and not as a constant) to avoid been saved in checkpoints
                self.model.content_article_embeddings_matrix: self.content_article_embeddings_matrix
        ...
        print("Before run total time: %f"%(time()-start))
        return tf.train.SessionRunArgs(fetches=fetches,
                                       feed_dict=feed_dict)
    
    

    def evaluate_and_update_streaming_metrics_last(self, clf, users_ids, clicked_items,save_path, next_item_labels, eval_negative_items):
        tf.logging.info('Evaluating benchmark: {}'.format(clf.get_description()))
        clf_metrics = clf.evaluate(users_ids, clicked_items, next_item_labels, save_path, topk=self.eval_metrics_top_n,
    ...
    def after_run(self, run_context, run_values): 
        if self.mode != tf.estimator.ModeKeys.PREDICT:
            ...
            if self.mode == tf.estimator.ModeKeys.EVAL:
                ...
                #PRINT SESSIONID, USERID, PREDICTED_ITEMS_ID
                #Minh
                save_path = ""
                if tf.flags.FLAGS['save_eval_per_sessions'].value:
                    model_path = tf.flags.FLAGS['model_dir'].value+tf.flags.FLAGS['current_time'].value+'/'
                    # current_time = datetime.datetime.now().strftime('%Y%m%d%H%M%S/')
                    trainning_step = str(self.global_eval_hour_id)+'/'
                    save_path = model_path+trainning_step
                    write_eval_data(sessions_ids, save_path+"session_id")
                    write_eval_data(clicked_items, save_path+"clicked_items")
                    write_eval_data(next_item_labels, save_path + "next_clicked_items")
                    predicted_item_convert = np.array(["|".join(",".join(map(str, pred_item)) for pred_item in predict) for predict in list(predicted_item_ids)])
                    write_eval_data(predicted_item_convert,
                                    save_path + "chaemeleon")
                ...
                for clf in self.bench_classifiers:                    
                    self.evaluate_and_update_streaming_metrics_last(clf, users_ids,
                                    clicked_items, save_path, next_item_labels, eval_batch_negative_items)
            ...
            if self.mode != tf.estimator.ModeKeys.PREDICT:
                ...
                batch_clicked_items = clicked_items
