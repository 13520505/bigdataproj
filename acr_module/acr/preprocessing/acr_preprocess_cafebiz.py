...
from ..utils import serialize, chunks, get_categ_encoder_from_values, encode_categ_feature, \
     deserialize, get_categ_encoder_from_values_second_time_category,get_categ_encoder_from_values_second_time_news_id
from .tokenization import tokenize_articles, nan_to_str, convert_tokens_to_int, get_words_freq, tokenize_articles_vietnameses
from .word_embeddings import load_word_embeddings_vietnamese, load_word_embeddings, process_word_embedding_for_corpus_vocab, save_word_vocab_embeddings
...
def flatten_list_series(series_of_lists):
    df =  pd.DataFrame(series_of_lists.apply(pd.Series).stack().reset_index(name='item'))['item']
    return df

def get_freq_values(series, min_freq=1):  #100
...
def process_cat_features(news_df):
    article_id_encoder = get_categ_encoder_from_values(news_df['id'].astype(int))
    # article_id_encoder = get_categ_encoder_from_values(news_df['id'])
    print('Articles - unique count {}'.format(len(article_id_encoder)))
    print(article_id_encoder)
    news_df['id_encoded'] = transform_categorical_column(news_df['id'].astype(int), article_id_encoder)
    ...
    # pickle_out = open("./acr_module/config/dict_cat.pickle", "wb")
    # pickle.dump(category0_encoder, pickle_out)
    # pickle_out.close()
    ...
    print("----------- encoder -------------")
    print(category0_encoder)
    print(news_df['category0_encoded'].unique())

    print('Category0 weights shape: {}'.format(category0_class_weights.shape))
    ...
    ## assert len(category0_class_weights) == len(category0_encoder), "weight must be same shape %d - %d" % (len(category0_class_weights),len(category0_encoder))
...
def load_input_csv(path):
    # news_df = pd.read_csv(path, sep = "\t", encoding = 'utf-8')

    news_df  = pd.read_parquet(path,  engine='fastparquet')
...
def custome_df(news_df):

    # news_df['created_at_ts'] =  pd.to_datetime(news_df['created_at_ts']).astype(int) // 10**9

    news_df['id'] = news_df['id'].apply(str)

    return news_df

def get_tkn_fn(max_words_length):
    def tokenize_vietnamese_article(text):
        words_tokenized = []
        for sentence in sent_tokenize(text ):
            words_tokenized.extend( word_tokenize(sentence ))
        return words_tokenized[:max_words_length] #Truncating to the maximum number of tokens
    return tokenize_vietnamese_article

def tokenize_vietnamese_article(text,max_words_length ):
    words_tokenized = []
    for sentence in sent_tokenize(text ):
        words_tokenized.extend( word_tokenize(sentence ))
    return words_tokenized[:max_words_length] #Truncating to the maximum number of tokens
...
def get_encoder_from_freq_values_in_list_column_second_time(series,acr_label_encoders_values,  min_freq=100):
    freq_values_counts_df = get_freq_values_series_of_lists(series, min_freq=min_freq)
    encoder = get_categ_encoder_from_values_second_time_category(freq_values_counts_df[freq_values_counts_df.columns[0]].unique(),
                                                        acr_label_encoders_values)
    return encoder


def map_cat(value, dict):
    if value in dict:
        return dict[value]


def process_cat_features_second_time(news_df, acr_label_encoders):
    article_id_encoder = get_categ_encoder_from_values_second_time_news_id(news_df['id'].astype(int), acr_label_encoders["article_id"])
    print('Articles - unique count {}'.format(len(article_id_encoder)))
    news_df['id_encoded'] = transform_categorical_column(news_df['id'].astype(int), article_id_encoder)


    category0_encoder = get_categ_encoder_from_values_second_time_category(news_df['category0'].unique(), acr_label_encoders['category0'])
    print('Category0 - unique count {}'.format(len(category0_encoder)))
    ...
    # Processing categorical list columns (Including only frequent categories)
    keywords_encoder = get_encoder_from_freq_values_in_list_column_second_time(news_df['keywords'], acr_label_encoders['keywords'])
    news_df['keywords_encoded'] = transform_categorical_list_column(news_df['keywords'], keywords_encoder)
    print('Keywords - freq. unique count {}'.format(len(keywords_encoder)))

    locations_encoder = get_encoder_from_freq_values_in_list_column_second_time(news_df['locations'], acr_label_encoders['locations'])
    news_df['locations_encoded'] = transform_categorical_list_column(news_df['locations'], locations_encoder)
    print('Locations - freq. unique count {}'.format(len(locations_encoder)))

    persons_encoder = get_encoder_from_freq_values_in_list_column_second_time(news_df['persons'], acr_label_encoders['persons'])
    news_df['persons_encoded'] = transform_categorical_list_column(news_df['persons'], persons_encoder)
    print('Persons - freq. unique count {}'.format(len(persons_encoder)))
...
def load_acr_preprocessing_label_encode(acr_label_encoders_path):
    (acr_label_encoders, labels_class_weights) = deserialize(acr_label_encoders_path)
    return acr_label_encoders, labels_class_weights

def load_acr_preprocessing_word_embedding(word_embedding_path):
    (word_vocab, word_embeddings_matrix) = deserialize(word_embedding_path)
    return word_vocab, word_embeddings_matrix
...
def main():
    ...
    current_time = get_date_time_current()
    ...
    # print('Total cat0', news_df.groupby(["category0_encoded"]).count())
    # print(news_df[news_df["category0_encoded"] == 40]['created_at_ts'].to_string())
    
    print("Saving news articles CSV to {}".format(args.output_articles_csv_path))
    news_df.to_csv(args.output_articles_csv_path + current_time, index=False)

    print('Tokenizing articles...')
    tokenized_articles = tokenize_articles(news_df['text_highlights'].values, tokenization_fn=get_tkn_fn(1000))
