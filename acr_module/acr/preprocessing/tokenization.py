...
def tokenize_articles_vietnameses(articles, tokenization_fn=None, max_words_length=1000, clean_str_fn=lambda x: x):
    if tokenization_fn == None:
        tokenized_articles = [tokenize_text(text, clean_str_fn) for text in articles]
    else:
        tokenized_articles = [tokenization_fn(text, max_words_length) for text in articles]
    return tokenized_articles
...
def convert_tokens_to_int_second_time(tokenized_articles, word_vocab):
    def token_to_int(token):
        return word_vocab[token] if token in word_vocab else 1
