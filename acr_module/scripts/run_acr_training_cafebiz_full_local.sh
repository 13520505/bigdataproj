...
MODEL_DIR='/data/tungtv/Code/dataset/dataset_cafebiz_full_45/tmp/chameleon/cafebiz/jobs/'${JOB_ID} && \
echo 'Running training job and outputing to '${MODEL_DIR} && \
python3 -m acr.acr_trainer_cafebiz_full \
	--model_dir ${MODEL_DIR} \
	--train_set_path_regex "${DATA_DIR}/articles_tfrecords/cafebiz_articles_*.tfrecord.gz" \
